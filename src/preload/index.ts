...
contextBridge.exposeInMainWorld("electron", {
  ...
  getTorrentFiles: (magnet: string) =>
    ...
    >,
  getQuarantineFiles: () =>
    ipcRenderer.invoke("getQuarantineFiles") as Promise<QuarantinedFile[]>,
  restoreFile: (id: string) => ipcRenderer.invoke("restoreFile", id),
  scanInstalledGame: (shop: GameShop, objectId: string) =>
    ipcRenderer.invoke("scanInstalledGame", shop, objectId),

  /* Security Dashboard */
  getSecurityEvents: () =>
    ipcRenderer.invoke("security:getSecurityEvents") as Promise<
      SecurityEvent[]
    >,
  getSecurityStats: () =>
    ipcRenderer.invoke("security:getDashboardStats") as Promise<SecurityStats>,
  onSecurityEvent: (cb: (event: SecurityEvent) => void) => {
    const listener = (
      _event: Electron.IpcRendererEvent,
      value: SecurityEvent
    ) => cb(value);
    ipcRenderer.on("on-security-event", listener);
    return () => ipcRenderer.removeListener("on-security-event", listener);
  },
  ...
  addDownloadSourceDirect: (url: string, name?: string) =>
    ipcRenderer.invoke("addDownloadSourceDirect", url, name),
  ...
  onBigScreenEscape: (cb: () => void) => {
    const listener = (_event: Electron.IpcRendererEvent) => cb();
    ipcRenderer.on("big-screen-escape", listener);
    return () => ipcRenderer.removeListener("big-screen-escape", listener);
  },
  onEnterBigScreen: (cb: () => void) => {
    const listener = (_event: Electron.IpcRendererEvent) => cb();
    ipcRenderer.on("enter-big-screen", listener);
    return () => ipcRenderer.removeListener("enter-big-screen", listener);
  },
