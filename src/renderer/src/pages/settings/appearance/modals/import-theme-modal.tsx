...
export const ImportThemeModal = ({
  ...
  const handleImportTheme = async () => {
    const themeUrl = `${THEME_WEB_STORE_URL}/themes/${themeName.toLowerCase()}/theme.css`;
    
    let themeCode: string;
    
    try {
      // Try to fetch the actual CSS content from the web store
      const response = await fetch(themeUrl);
      if (!response.ok) {
        throw new Error(`Failed to fetch theme CSS: ${response.statusText}`);
      }
      themeCode = await response.text();
    } catch (fetchError) {
      // If fetching fails, fallback to storing the URL
      logger.warn("Failed to fetch theme CSS, storing URL instead:", fetchError);
      themeCode = themeUrl;
    }
