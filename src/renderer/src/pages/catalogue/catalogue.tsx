...
export default function Catalogue() {
  ...
  const steamGenresMapping = useMemo<Record<string, string>>(() => {
    const genresLang = steamGenres[language] || steamGenres["en"];
    if (!genresLang) return {};

    return genresLang.reduce((prev, genre, index) => {
  ...
  const steamUserTagsFilterItems = useMemo(() => {
    const tagsLang = steamUserTags[language] || steamUserTags["en"];
    if (!tagsLang) return [];

    return Object.entries(tagsLang)
      .sort(([keyA], [keyB]) => keyA.localeCompare(keyB))
      .map(([key, value]) => ({
        label: key,
        value: value,
  ...
  const groupedFilters = useMemo(() => {
    ...
    return [
      ...
      ...filters.tags.map((tag) => ({
        label: Object.keys(steamUserTags[language] || steamUserTags["en"]).find(
  ...
  const filterSections = useMemo(() => {
    return [
      ...
      {
        title: t("developers"),
        items: Array.isArray(steamDevelopers)
      ...
      {
        title: t("publishers"),
        items: Array.isArray(steamPublishers)
    ...
    <div className="catalogue" ref={cataloguePageRef}>
      <div className="catalogue__header">
        ...
        <Button
          type="button"
          theme="outline"
          className="catalogue__surprise-button"
