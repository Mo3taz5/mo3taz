...
export function AchievementsContent({
  ...
  useEffect(() => {
    ...
    const step = (time: number) => {
      ...
      if (hasOverflow && time >= pausedUntil) {
        const delta = 0.45 * direction;
        const next = Math.max(0, Math.min(maxScrollTop, container.scrollTop + delta));
        container.scrollTop = next;

        if (next <= 0.5) {
          direction = 1;
          pausedUntil = time + 1100;
        } else if (next >= maxScrollTop - 0.5) {
          direction = -1;
          pausedUntil = time + 1400;
        }
      }
  ...
  }, [achievements, comparedAchievements, otherUser]);
