...
export function GameContextMenu({
  ...
  const [showCreateCollectionModal, setShowCreateCollectionModal] =
    useState(false);
  const [isScanningGame, setIsScanningGame] = useState(false);
  ...
  const handleScanInstalledGame = async () => {
    if (!game.executablePath || isDeleting || isScanningGame) {
      return;
    }

    setIsScanningGame(true);

    try {
      const result = await window.electron.scanInstalledGame(
        game.shop,
        game.objectId
      );

      if (result.status === "threats-found") {
        showErrorToast(
          t("security_scan_threat_detected_description", {
            defaultValue:
              "The scan found suspicious files. Open Quarantine History to review them.",
      ...
      if (result.status === "clean") {
        showSuccessToast(
          t("security_scan_complete_description", {
            defaultValue: "The game scan completed and no issues were found.",
          })
        );
        return;
      }
      ...
      showErrorToast(
        t("security_scan_unavailable_description", {
          defaultValue:
            "The scan could not be completed on this device or for this game.",
        })
      );
    } catch (error) {
      void error;
      showErrorToast(
        t("security_scan_failed_description", {
          defaultValue: "The game scan failed.",
        })
      );
    } finally {
      setIsScanningGame(false);
  ...
  const items: ContextMenuItemData[] = [
    {
      id: "show-game-page",
      label: t("show_game_page", {
        defaultValue: "Show game page",
      }),
      icon: <FileDirectoryIcon size={16} />,
...

    {
      ...
      submenu: [
        ...
        ...(game.executablePath
          ? [
              {
                id: "scan-installed-game",
                label: t("scan_installed_game", {
                  defaultValue: "Scan installed game",
                }),
                icon: <ShieldCheckIcon size={16} />,
                onClick: () => {
                  void handleScanInstalledGame();
                },
                disabled: isDeleting || isScanningGame,
              },
            ]
          : []),
