...
export function Sidebar() {
  ...
  >([]);
  const [_chaosMode, setChaosMode] = useState(false);
  const {
    collections,
  ...
  const handleGifClick = () => {
    ...
    setTimeout(() => {
      setIsGifExploding(false);
    }, 600);

    // Calculate the new count properly (prev + 1)
    const newCount = gifClickCount + 1;
    setGifClickCount(newCount);

    // Easter egg stages with increasing chaos - NO RESET, count keeps going!
    if (newCount >= 50) {
      // 50 clicks: ABSOLUTE CHAOS - everything everywhere all at once
      setGifClickCount(0);
      setChaosMode(true);
      showSuccessToast(
        t("gif_easter_egg_50", {
          defaultValue: "🦂💀 YOU BROKE IT! CHAOS MODE! 💀🦂",
        })
      );

      // Mix of everything
      const emojis = ["🦂", "🔥", "💀", "⚡", "🌪️", "🎉", "✨", "🎊"];
      const chaosArray = Array.from({ length: 100 }, (_, i) => ({
        id: i,
        x: Math.random() * 100,
        delay: Math.random() * 4,
        emoji: emojis[Math.floor(Math.random() * emojis.length)],
      }));
      setScorpions(chaosArray);

      // Rainbow background
      document.documentElement.style.setProperty("--chaos-bg", "1");
      document.body.style.animation = "screen-shake 0.3s ease-in-out infinite";

      setTimeout(() => {
        setChaosMode(false);
        document.documentElement.style.removeProperty("--chaos-bg");
        document.body.style.animation = "";
        setScorpions([]);
      }, 5000);
    } else if (newCount >= 40) {
      // 40 clicks: Fire + scorpions
      setGifClickCount(0);
      showSuccessToast(
        t("gif_easter_egg_40", { defaultValue: "🦂🔥 BURNING SCORPIONS! 🔥🦂" })
      );

      const fireArray = Array.from({ length: 70 }, (_, i) => ({
        id: i,
        x: Math.random() * 100,
        delay: Math.random() * 3,
        emoji: i % 2 === 0 ? "🦂" : "🔥",
      }));
      setScorpions(fireArray);

      setTimeout(() => {
        setScorpions([]);
      }, 4000);
    } else if (newCount >= 30) {
      // 30 clicks: Lightning storm
      setGifClickCount(0);
      showSuccessToast(
        t("gif_easter_egg_30", { defaultValue: "🦂⚡ LIGHTNING STORM! ⚡🦂" })
      );

      const lightningArray = Array.from({ length: 60 }, (_, i) => ({
        id: i,
        x: Math.random() * 100,
        delay: Math.random() * 3,
        emoji: i % 3 === 0 ? "🦂" : "⚡",
      }));
      setScorpions(lightningArray);

      setTimeout(() => {
        setScorpions([]);
      }, 3500);
    } else if (newCount >= 25) {
      // 25 clicks: Screen shake + scorpion rain
      setGifClickCount(0);
      showSuccessToast(
        t("gif_easter_egg_25", {
          defaultValue: "🦂🔥 CHAOS MODE ACTIVATED! 🔥🦂",
        })
      );

      const scorpionArray = Array.from({ length: 60 }, (_, i) => ({
        id: i,
        x: Math.random() * 100,
        delay: Math.random() * 3,
        emoji: "🦂",
      }));
      setScorpions(scorpionArray);
      ...
      // 20 clicks: Double scorpion army
      setGifClickCount(0);
      showSuccessToast(
        t("gif_easter_egg_20", {
          defaultValue: "🦂🦂 DOUBLE SCORPION ARMY! 🦂🦂",
        })
      );

      const doubleArray = Array.from({ length: 50 }, (_, i) => ({
        id: i,
        x: Math.random() * 100,
        delay: Math.random() * 2.5,
        emoji: "🦂",
      }));
      setScorpions(doubleArray);

      setTimeout(() => {
        setScorpions([]);
      }, 3500);
    } else if (newCount >= 15) {
      // 15 clicks: Scorpion madness
      setGifClickCount(0);
      showSuccessToast(
        t("gif_easter_egg_15", {
          defaultValue: "🦂🦂🦂 SCORPION MADNESS! 🦂🦂🦂",
        })
      );

      const madnessArray = Array.from({ length: 40 }, (_, i) => ({
        id: i,
        x: Math.random() * 100,
        delay: Math.random() * 2,
        emoji: "🦂",
      }));
      setScorpions(madnessArray);

      setTimeout(() => {
        setScorpions([]);
      }, 3000);
    } else if (newCount >= 10) {
      // 10 clicks: Original scorpion rain
      setGifClickCount(0);
      showSuccessToast(
        t("gif_easter_egg", { defaultValue: "🦂 SCORPION RAIN! 🦂" })
      );
      ...
      // 7 clicks: Lucky number
      showSuccessToast(
        t("gif_easter_egg_7", {
          defaultValue: "🦂 Lucky 7! Just 3 more for a surprise!",
        })
      );
    } else if (newCount >= 5) {
      // 5 clicks: Halfway there
      showSuccessToast(
        t("gif_easter_egg_5", {
          defaultValue: "🦂 Halfway there... keep clicking!",
        })
      );
    } else if (newCount >= 3) {
      // 3 clicks: Encouragement
      showSuccessToast(
        t("gif_easter_egg_3", {
          defaultValue: "🦂 Click 10 times for a surprise!",
        })
      );
    }
  };
  ...
  return (
    ...
    >
      {/* Scorpion rain overlay */}
      {scorpions.length > 0 && (
        <div className="scorpion-rain">
          {scorpions.map((scorpion) => (
            <div
              ...
              style={{
                left: `${scorpion.x}%`,
                animationDelay: `${scorpion.delay}s`,
              }}
            >
              {scorpion.emoji || "🦂"}
            </div>
      ...
      <div className="sidebar__container">
        ...
        <div
          className={`sidebar__brand-gif ${isGifExploding ? "sidebar__brand-gif--exploding" : ""}`}
          onClick={handleGifClick}
          role="button"
          tabIndex={0}
          onKeyDown={(e) => e.key === "Enter" && handleGifClick()}
        >
          <img src={outputGif} alt="MO3TAZ" />
        </div>
          ...
          <section className="sidebar__section">
            <ul className="sidebar__menu">
              {routes.map(({ nameKey, path, render }) => {
                ...
                const activeDownloadCount = lastPacket ? 1 : 0;
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {render()}
                      <span>{t(nameKey)}</span>
                      {isDownloads && activeDownloadCount > 0 && (
                        <span className="sidebar__menu-item-badge">
                          {activeDownloadCount}
                        </span>
                      )}
                    </button>
