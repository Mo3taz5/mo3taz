...
type GameSecurityScanStatus =
  | "clean"
  | "threats-found"
  | "unavailable"
  | "error";

interface GameSecurityFinding {
  filePath: string | null;
  signature: string | null;
  category: "Low Risk" | "Requires Investigation";
  reason: string;
  description: string;
  raw: string;
}

interface GameSecurityScanResult {
...
declare global {
  ...
  interface Electron {
    ...
    >;
    getQuarantineFiles: () => Promise<QuarantinedFile[]>;
    restoreFile: (id: string) => Promise<{ restoredPath: string }>;
    scanInstalledGame: (
      shop: GameShop,
      objectId: string
    ...
    onSecurityEvent: (cb: (event: any) => void) => () => void;
    ...
    onBigScreenEscape: (cb: () => void) => () => Electron.IpcRenderer;
    onEnterBigScreen: (cb: () => void) => () => Electron.IpcRenderer;
  ...
  interface Window {
    electron: Electron;
    api: Pick<Electron, "getQuarantineFiles" | "restoreFile">;
  }
}
