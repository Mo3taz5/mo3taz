...
import { Sidebar, BottomPanel, Header, Toast, DownloadStatusBar } from "@renderer/components";
...
import BigScreenLayout from "./pages/big-screen/BigScreenLayout";
import "./app.scss";
...
export function App() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      levelDBService.get("appearancePreferences", null, "json"),
      updateLibrary(),
    ]).then(([preferences, appearancePrefs]) => {
      dispatch(setUserPreferences(preferences as UserPreferences | null));
      if (appearancePrefs && typeof appearancePrefs === "object") {
        const prefs = appearancePrefs as Record<string, unknown>;
        if (prefs.defaultViewMode === "big") {
          dispatch(setControllerMode(true));
        }
      }
    });
  ...
  const setupWorkWonders = useCallback(
    async (token?: string, locale?: string) => {
      ...
      // HIDDEN: Knowledge widget (contains Hydra branding)
  ...
  useEffect(() => {
    const onClick = async (event: MouseEvent) => {
      ...
      if ($helpCenterTarget) {
        ...
        if (articleId) {
          // HIDDEN: Knowledge widget article (contains Hydra branding)
  ...
  const onSignIn = useCallback(() => {
    console.log("[App] onSignIn triggered");
    // Track sign-in event
    window.electron.analytics.track("sign_in").catch(() => {});
    
    fetchUserDetails().then((response) => {
      console.log("[App] fetchUserDetails returned:", response);
      if (response) {
        updateUserDetails(response);
        void Promise.all([window.electron.refreshLibraryAssets(), updateLibrary()]);
        showSuccessToast(t("successfully_signed_in"));
      } else {
        console.warn("[App] fetchUserDetails returned null/undefined");
      }
    }).catch((err) => {
      console.error("[App] onSignIn error:", err);
    });
  }, [fetchUserDetails, t, showSuccessToast, updateUserDetails, updateLibrary]);
  ...
  useEffect(() => {
    const listeners = [
      ...
      window.electron.onAccountUpdated(() => {
        void Promise.all([window.electron.refreshLibraryAssets(), updateLibrary()]);
      }),
      ...
      window.electron.onSignOut(() => {
        ...
        window.electron.analytics.track("sign_out").catch(() => {});
        clearUserDetails();
      }),
  ...
  // Listen for exit-big-screen event dispatched from BigScreenLayout
  useEffect(() => {
    const handleExit = () => {
      dispatch(toggleControllerMode());
    };
    window.addEventListener("exit-big-screen", handleExit);
    return () => window.removeEventListener("exit-big-screen", handleExit);
  }, [dispatch]);

  // Note: Big Screen mode is now handled directly in the initial load effect above
  // via setControllerMode(true) when defaultViewMode preference is "big"
