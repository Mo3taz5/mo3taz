...
import { logSecurityEvent } from "@main/events/security";
...
const launchNatively = async (
  ...
  logger.info(`[Security] launchNatively called: ${executablePath}`);
  ...
  // Always use sandboxed launch (even with no wrappers)
  // Try protected launch with Job Object sandbox
  let spawnedPid: number | null = null;
  let needsFallback = false;

  logger.info("[Security] Before spawnIsolatedProcess try block");
  try {
    logger.info(
      "[Security] Inside spawnIsolatedProcess try, about to call ProcessIsolation.spawnIsolatedProcess"
    );
    ...
    const { process: isoProcess, context } =
      await ProcessIsolation.spawnIsolatedProcess(
    ...
    spawnedPid = isoProcess.pid ?? null;

    // Start runtime monitoring for the game process
    if (spawnedPid) {
      RuntimeMonitor.startMonitoring(spawnedPid);
      logger.info(
        `[Security] Protected launch: PID ${spawnedPid}, Job: ${context.jobObjectName}`
      );
      logSecurityEvent(
        "low",
        "allow",
        "ProcessIsolation",
        `Game launched with sandbox: PID ${spawnedPid}`,
        { pid: spawnedPid, jobName: context.jobObjectName }
      );
    }
  } catch (error) {
    // Fail-open: fallback to regular spawn if isolation fails
    logger.warn("[Security] Isolation failed, falling back to regular spawn", {
      error,
    });
    needsFallback = true;
  }

  // Fallback to regular spawn if protected launch failed
  if (needsFallback) {
    const envWithStrings: Record<string, string> = {
      ...(process.env as Record<string, string>),
      ...resolvedLaunchCommand.env,
    ...
    processRef.unref();
    spawnedPid = processRef.pid ?? null;
    logger.info(`[Security] Regular launch (fallback): PID ${spawnedPid}`);
    if (spawnedPid) {
      logSecurityEvent(
        "low",
        "allow",
        "ProcessIsolation",
        `Game launched without sandbox: PID ${spawnedPid}`,
        { pid: spawnedPid }
      );
    }
  }
...
export const launchGame = async (options: LaunchGameOptions): Promise<void> => {
  ...
  if (process.platform === "linux") {
    ...
    await launchNatively(parsedPath, launchOptions, useMangohud, useGamemode);
    return;
  }
