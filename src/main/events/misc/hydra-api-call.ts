...
const hydraApiCall = async (
  ...
  try {
    ...
    switch (method) {
      case "get":
        request = HydraApi.get(url, { ...(options || {}), params: params as Record<string, any> });
        break;
      case "post":
    ...
    // Don't throw if it's just a configuration error - return empty data instead
    if (errorMessage?.includes("Invalid URL") || errorMessage?.includes("API URL not configured")) {
      return null;
    }
