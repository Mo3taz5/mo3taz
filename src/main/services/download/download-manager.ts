...
import {
  ...
  publishGameSecurityThreatNotification,
} from "../notifications";
...
import { GameSecurityScanner } from "../game-security-scanner";
import {
  QuarantineManager,
  buildQuarantinePath,
} from "../quarantine/quarantine-manager";
...
export class DownloadManager {
  ...
  private static async getDownloadStatusFromJs(): Promise<DownloadProgress | null> {
    ...
    if (this.isPreparingDownload) {
      logger.log(`[DownloadManager] getDownloadStatusFromJs: isPreparingDownload=true for ${downloadId}`);
      try {
    ...
    if (!this.jsDownloader) {
      logger.log(`[DownloadManager] getDownloadStatusFromJs: jsDownloader is null for ${downloadId}`);
      return null;
    }

    const status = this.jsDownloader.getDownloadStatus();
    logger.log(`[DownloadManager] getDownloadStatusFromJs: jsDownloader status=${status?.status ?? 'null'}, progress=${status?.progress ?? 0}`);
  ...
  public static async watchDownloads() {
    try {
      if (this.downloadingGameId) {
        logger.log(`[DownloadManager] watchDownloads polling: downloadingGameId=${this.downloadingGameId}`);
      }
      const status = await this.getDownloadStatus();

      if (!status) {
        // No active download, clear progress
        if (WindowManager.mainWindow) {
          WindowManager.mainWindow.setProgressBar(-1);
        }
        // Log why no status
        if (this.downloadingGameId) {
          logger.log(`[DownloadManager] watchDownloads: No status but downloadingGameId=${this.downloadingGameId}, usingJsDownloader=${this.usingJsDownloader}, isPreparing=${this.isPreparingDownload}, jsDownloader=${!!this.jsDownloader}`);
        }
        return;
      }
    ...
    if (!download || !game) {
      logger.warn(`[DownloadManager] No download or game found for ${gameId}`);
      return;
    }

    logger.info(
      `[DownloadManager] Progress: ${gameId} - ${(progress * 100).toFixed(1)}% - Status: ${download.status}`
    );
    ...
    if (isComplete) {
      logger.info(`[DownloadManager] Download complete for ${gameId}`);
      await this.handleDownloadCompletion(download, game, gameId);
    }
    } catch (err) {
      logger.error(`[DownloadManager] watchDownloads error:`, err);
    }
  }
  ...
  private static async handleDownloadCompletion(
    ...
    let shouldSeed = await this.updateDownloadStatus(
      download,
      gameId,
    ...
    const scanPath = download.folderName
      ? path.join(download.downloadPath, download.folderName)
      : download.downloadPath;

    if (userPreferences?.scanAfterDownloadComplete && scanPath && fs.existsSync(scanPath)) {
      const scanResult = await GameSecurityScanner.scanPath(scanPath);

      if (scanResult.status === "threats-found") {
        logger.error("[DownloadManager] Security scan detected a threat", {
          gameId,
          scanPath,
        ...
        await Promise.all(
          scanResult.findings.map((finding) => {
            ...
            return QuarantineManager.registerFile({
              originalPath: finding.filePath,
              quarantinePath: buildQuarantinePath(finding.filePath, scanPath),
              scanPath,
        ...
        if (download.downloader === Downloader.Torrent && download.status === "seeding") {
          await this.pauseSeeding(gameId).catch((error) => {
            logger.error(
              "[DownloadManager] Failed to pause seeding after security threat",
              error
            );
          });
        }
        ...
        await downloadsSublevel.put(gameId, {
          ...download,
          status:
            download.downloader === Downloader.Torrent && download.status === "seeding"
              ? "complete"
              : download.status,
          shouldSeed: false,
          queued: false,
        ...
        await publishGameSecurityThreatNotification(
          game,
          scanResult.details,
          scanResult.findings
        );
      ...
      if (scanResult.status === "clean") {
        logger.info("[DownloadManager] Security scan completed cleanly", {
          gameId,
          scanPath,
          scanner: scanResult.scanner,
        });
      } else if (scanResult.status !== "threats-found") {
        logger.warn("[DownloadManager] Security scan could not complete", {
          gameId,
          scanPath,
    ...
    } else if (!userPreferences?.scanAfterDownloadComplete) {
      logger.info("[DownloadManager] Skipping post-download security scan", {
        gameId,
      });
    }
  ...
  private static async processNextQueuedDownload() {
    const downloads = await downloadsSublevel
      ...
      .then((games) =>
        orderBy(
          ...
          ["timestamp"],
          ["asc"] // Changed from "desc" to "asc" - queue should be FIFO (First In, First Out)
        )
      );
    ...
    if (nextItemOnQueue) {
      logger.log(`[DownloadManager] Starting next queued download: ${nextItemOnQueue.objectId}`);
      this.resumeDownload(nextItemOnQueue);
    } else {
      logger.log("[DownloadManager] No queued downloads found, clearing active download state");
  ...
  static async startDownload(download: Download) {
    ...
    logger.log(`[DownloadManager] startDownload called for ${downloadId}, isHttp=${isHttp}, downloader=${download.downloader}`);
    ...
    if (isHttp) {
      ...
      logger.log(`[DownloadManager] State set: downloadingGameId=${downloadId}, isPreparing=true, usingJs=true`);

      try {
        if (download.downloader === Downloader.AllDebrid) {
          logger.log(`[DownloadManager] AllDebrid download path`);
          ...
          this.isPreparingDownload = false;
          logger.log(`[DownloadManager] AllDebrid batch initialized, starting batch`);
          void this.runAllDebridBatch();
        } else {
          this.allDebridBatch = null;
          logger.log(`[DownloadManager] Getting JS download options...`);
          const options = await this.getJsDownloadOptions(download);

          if (!options) {
            logger.error(`[DownloadManager] Failed to get download options for JS downloader`);
            this.isPreparingDownload = false;
            this.usingJsDownloader = false;
            this.downloadingGameId = null;
          ...
          logger.log(`[DownloadManager] Options received: url=${options.url.substring(0, 100)}..., savePath=${options.savePath}`);
          ...
          this.isPreparingDownload = false;
          logger.log(`[DownloadManager] JsDownloader created, isPreparing=false, starting download...`);
          ...
          logger.log(`[DownloadManager] startDownload completed for ${downloadId}, download running in background`);
        }
      } catch (err) {
        logger.error(`[DownloadManager] Exception in startDownload:`, err);
        this.isPreparingDownload = false;
        this.usingJsDownloader = false;
        this.downloadingGameId = null;
        this.allDebridBatch = null;
