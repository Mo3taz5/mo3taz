...
import type { GameSecurityFinding } from "../game-security-scanner";
...
export const publishGameSecurityThreatNotification = async (
  game: Game,
  details: string | null,
  findings: GameSecurityFinding[] = []
) => {
  const title = t("security_scan_threat_detected_title", {
    defaultValue: "Potential malware detected",
  });
  ...
  const findingsSummary =
    findings.length > 0
      ? findings
          .slice(0, 5)
          .map((finding) => {
            const fileLabel = finding.filePath ?? "unknown file";
            const signatureLabel = finding.signature ?? "unknown signature";
            const categoryLabel = finding.category
              ? ` (${finding.category})`
              : "";
            return `${fileLabel}: ${signatureLabel}${categoryLabel} - ${finding.reason}`;
          })
          .join("\n")
      : null;

  const body = findingsSummary
    ? details
      ? t("security_scan_threat_detected_description", {
          defaultValue:
            "The launcher quarantined suspicious files in \"{{title}}\".\n\nWhy it was flagged:\n{{findingsSummary}}\n\n{{details}}",
          title: game.title,
          details,
          findingsSummary,
        })
    ...
    : details
      ? t("security_scan_threat_detected_description", {
          defaultValue:
            "The launcher quarantined suspicious files in \"{{title}}\". {{details}}",
          title: game.title,
          details,
        })
      : t("security_scan_threat_detected_description", {
