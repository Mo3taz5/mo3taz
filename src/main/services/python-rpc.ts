...
export class PythonRPC {
  ...
  public static async spawn(
    ...
    if (app.isPackaged) {
      ...
      if (!fs.existsSync(binaryPath)) {
        dialog.showErrorBox(
          "Fatal",
          "MO3TAZ Python RPC binary not found. Please check if it has been removed by Windows Defender."
        );

        app.quit();
        throw new Error(`MO3TAZ Python RPC binary not found at ${binaryPath}`);
      }
  ...
  private static resolvePythonExecutable() {
    ...
    throw new Error(
      "Python executable not found. Set MO3TAZ_PYTHON_BIN or install python3/python."
    );
  }
